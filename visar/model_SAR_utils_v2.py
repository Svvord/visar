...
#from model_training_utils_v2 import optimize_SVR, optimize_RidgeCV

#from TRACE_utils import Parameters

#TASK_LAYERS = ['Dense_%d/Dense_%d/Relu:0' + (10 + n_bypass * 2 * idx)
#               for idx in range(n_tasks)]
#SHARE_LAYER = 'Dense_7/Dense_%d/Relu:0'
...
def calculate_gradients_baseline(train_dataset, model_type):
    ...
    if model_type == 'SVR':
        model = model_training_utils_v2.optimize_SVR(X_new, train_dataset.y)
        out = model.coef_
    elif model_type == 'RidgeCV':
        model = model_training_utils_v2.optimize_RidgeCV(X_new, train_dataset.y)
